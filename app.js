...
const FAV_KEY = "warehouse_favorites_v1";
const RECENT_KEY = "warehouse_recents_v1";
const USAGE_KEY = "warehouse_usage_v1";
...
let query = "";
let chipMode = "all"; // all | fav | recent
...
const chipRecent = document.getElementById("chipRecent");
...
let usage = loadObj(USAGE_KEY);
let recents = loadObj(RECENT_KEY); // {id: timestamp}
...
function filteredSheets() {
  ...
  let list = SHEETS.filter((s) => s.category === activeCategory);

  // chip filters
  if (chipMode === "fav") {
    list = list.filter(s => favorites.has(s.id));
  } else if (chipMode === "recent") {
    // keep items that exist in recents map
    list = list.filter(s => recents[s.id]);
    // default sort for recent
  }

  // query filtering (title/desc/tags)
  list = list.filter((s) => {
    const tags = (s.tags || []).map(t => String(t).toLowerCase());
    const hay = [s.title, s.description || "", tags.join(" "), s.updatedAt || ""].join(" ").toLowerCase();

    if (tagTokens.length) {
      for (const t of tagTokens) if (!tags.includes(t)) return false;
    }
  ...
  const byUpdated = (a,b) => (String(b.updatedAt||"") > String(a.updatedAt||"")) ? 1 : (String(b.updatedAt||"") < String(a.updatedAt||"") ? -1 : 0);
  const byAZ = (a,b) => a.title.localeCompare(b.title, "id", { sensitivity:"base" });
  const byUsed = (a,b) => (Number(usage[b.id]||0) - Number(usage[a.id]||0));
  const byRecent = (a,b) => (Number(recents[b.id]||0) - Number(recents[a.id]||0));
  ...
  if (chipMode === "recent") {
    ...
    // relevance: keep order (newest first by updated if exists)
    list.sort(byUpdated);
  }
...
function cardTemplate(item) {
  ...
  const updated = item.updatedAt ? `<span class="badge">Update: ${esc(item.updatedAt)}</span>` : "";
  ...
  <article class="card">
    <div class="card-body">
      <div class="card-top">
        ...
        <div class="row">
          <button class="favbtn ${favOn ? "on" : ""}" data-fav="${esc(item.id)}" type="button" title="Favorit">${favOn ? "★" : "☆"}</button>
          <button class="btn" data-open="${esc(item.url)}" data-id="${esc(item.id)}" type="button">
      ...
      <div class="row" style="margin-top:10px; justify-content: space-between;">
        <span class="muted small">${usage[item.id] ? `Dibuka: <b>${usage[item.id]}</b>x` : ""}</span>
        <button class="smallbtn" type="button" data-copy="${esc(item.url)}">Copy link</button>
      </div>
    </div>
  </article>`;
}
...
function setChip(mode){
  chipMode = mode;
  [chipAll, chipFav, chipRecent].forEach(btn => {
    if(!btn) return;
    btn.classList.toggle("active", btn.getAttribute("data-chip") === mode);
  });
  renderAll();
}
document.addEventListener("click", (e) => {
  ...
  if(chip){
    setChip(chip.getAttribute("data-chip") || "all");
  }
});
if(sortSelect){
...
function init(){
  ...
  if(boot && bootBar){
    ...
    const t1 = setTimeout(()=> { p = 46; bootBar.style.width = p + "%"; }, 180);
    const t2 = setTimeout(()=> { p = 72; bootBar.style.width = p + "%"; }, 360);
    const t3 = setTimeout(()=> { p = 100; bootBar.style.width = p + "%"; }, 520);
    const t4 = setTimeout(()=> { boot.classList.add("hide"); boot.setAttribute("aria-hidden","true"); }, 720);
    // no cleanup needed for static page
  }
